...
- **智能扫描** — 一键扫描配置的监控目录中的文件（默认 Downloads / Desktop / Documents）
...
- 打开应用 **「设置」**，可编辑 **监控目录** 列表与 **整理目标**（默认整理到 `~/Organized`，Windows 为当前用户主目录下的 `Organized`）。保存后会写入 `backend/config.yaml`。
...
1. 在 **「设置」** 中按需调整监控目录与整理目标（或使用默认）
2. 在 **「整理文件」** 点击「**扫描目录**」，扫描当前配置下的所有监控目录
